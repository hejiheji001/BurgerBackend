...
try
{
    Log.Information("Configuring web host ({ApplicationContext})...", Listing.API.Program.AppName);

    var host = CreateHostBuilder(configuration, args);

    Log.Information("Applying migrations ({ApplicationContext})...", Listing.API.Program.AppName);

    host.MigrateDbContext<ListingContext>((context, services) =>
        {
            var logger = services.GetService<ILogger<ListingInitializer>>();
            new ListingInitializer().SeedAsync(context, logger).Wait();
        })
        .MigrateDbContext<IntegrationEventLogContext>((_, __) => { });

    Log.Information("Starting web host ({ApplicationContext})...", Listing.API.Program.AppName);
    host.Run();
...
{
    Log.Fatal(ex, "Program terminated unexpectedly ({ApplicationContext})!", Listing.API.Program.AppName);
    return 1;
}
finally
{
...
ILogger CreateSerilogLogger(IConfiguration configuration)
{
    ...
    loggerBuilder.Enrich.WithProperty("ApplicationContext", Listing.API.Program.AppName);
...
namespace Listing.API
{
    ...
    {
        public static string AppName => "Listing.API";
    }
}